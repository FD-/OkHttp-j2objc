...
public final class OkUtil {
  ...
  public static final OkResponseBody EMPTY_RESPONSE = OkResponseBody.create(null, EMPTY_BYTE_ARRAY);
  public static final OkRequestBody EMPTY_REQUEST = OkRequestBody.create(null, EMPTY_BYTE_ARRAY);

  private static final OkioByteString UTF_8_BOM = OkioByteString.decodeHex("efbbbf");
  private static final OkioByteString UTF_16_BE_BOM = OkioByteString.decodeHex("feff");
  private static final OkioByteString UTF_16_LE_BOM = OkioByteString.decodeHex("fffe");
  private static final OkioByteString UTF_32_BE_BOM = OkioByteString.decodeHex("0000ffff");
  private static final OkioByteString UTF_32_LE_BOM = OkioByteString.decodeHex("ffff0000");
   ...
   */
  public static boolean discard(OkioSource source, int timeout, TimeUnit timeUnit) {
    try {
   ...
   */
  public static boolean skipAll(OkioSource source, int duration, TimeUnit timeUnit) throws IOException {
    ...
    try {
      OkioBuffer skipBuffer = new OkioBuffer();
  ...
  public static String hostHeader(OkHttpUrl url, boolean includeDefaultPort) {
    String host = url.host().contains(":")
            ? "[" + url.host() + "]"
            : url.host();
    return includeDefaultPort || url.port() != OkHttpUrl.defaultPort(url.scheme())
            ? host + ":" + url.port()
            : host;
  }
  ...
  public static Charset bomAwareCharset(OkioBufferedSource source, Charset charset) throws IOException {
