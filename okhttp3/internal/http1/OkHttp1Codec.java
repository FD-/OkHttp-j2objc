...
import okio.OkioForwardingTimeout;
import okio.Okio;
...
import static okhttp3.internal.OkUtil.checkOffsetAndCount;
 ...
 */
public final class OkHttp1Codec implements OkHttpCodec {
  ...
  final OkioBufferedSource source;
  final OkioBufferedSink sink;
  int state = STATE_IDLE;
  private long headerLimit = HEADER_LIMIT;

  public OkHttp1Codec(OkHttpClient client, OkStreamAllocation streamAllocation, OkioBufferedSource source,
  ...
  @Override public void writeRequestHeaders(OkRequest request) throws IOException {
    String requestLine = OkRequestLine.get(
  ...
  @Override public OkResponseBody openResponseBody(OkResponse response) throws IOException {
    ...
    if (!OkHttpHeaders.hasBody(response)) {
      OkioSource source = newFixedLengthSource(0);
      return new OkRealResponseBody(contentType, 0, Okio.buffer(source));
    }

    if ("chunked".equalsIgnoreCase(response.header("Transfer-Encoding"))) {
      OkioSource source = newChunkedSource(response.request().url());
      return new OkRealResponseBody(contentType, -1L, Okio.buffer(source));
    }

    long contentLength = OkHttpHeaders.contentLength(response);
    if (contentLength != -1) {
      OkioSource source = newFixedLengthSource(contentLength);
      return new OkRealResponseBody(contentType, contentLength, Okio.buffer(source));
    }

    return new OkRealResponseBody(contentType, -1L, Okio.buffer(newUnknownLengthSource()));
  }
  ...
  public void writeRequest(OkHeaders headers, String requestLine) throws IOException {
  ...
  @Override public OkResponse.Builder readResponseHeaders(boolean expectContinue) throws IOException {
    ...
    try {
      OkStatusLine statusLine = OkStatusLine.parse(readHeaderLine());

      OkResponse.Builder responseBuilder = new OkResponse.Builder()
  ...
  public OkHeaders readHeaders() throws IOException {
    OkHeaders.Builder headers = new OkHeaders.Builder();
    // parse the result headers until the first blank line
    for (String line; (line = readHeaderLine()).length() != 0; ) {
      OkInternal.instance.addLenient(headers, line);
    }
    return headers.build();
  }

  public OkioSink newChunkedSink() {
    if (state != STATE_OPEN_REQUEST_BODY) throw new IllegalStateException("state: " + state);
    state = STATE_WRITING_REQUEST_BODY;
    return new ChunkedSink();
  }

  public OkioSink newFixedLengthSink(long contentLength) {
    if (state != STATE_OPEN_REQUEST_BODY) throw new IllegalStateException("state: " + state);
    state = STATE_WRITING_REQUEST_BODY;
    return new FixedLengthSink(contentLength);
  }

  public OkioSource newFixedLengthSource(long length) throws IOException {
    if (state != STATE_OPEN_RESPONSE_BODY) throw new IllegalStateException("state: " + state);
    state = STATE_READING_RESPONSE_BODY;
    return new FixedLengthSource(length);
  }

  public OkioSource newChunkedSource(OkHttpUrl url) throws IOException {
    if (state != STATE_OPEN_RESPONSE_BODY) throw new IllegalStateException("state: " + state);
    state = STATE_READING_RESPONSE_BODY;
    return new ChunkedSource(url);
  }

  public OkioSource newUnknownLengthSource() throws IOException {
    if (state != STATE_OPEN_RESPONSE_BODY) throw new IllegalStateException("state: " + state);
  ...
  /**
   * Sets the delegate of {@code timeout} to {@link OkioTimeout#NONE} and resets its underlying timeout
   * to the default configuration. Use this to avoid unexpected sharing of timeouts between pooled
   * connections.
   */
  void detachTimeout(OkioForwardingTimeout timeout) {
    OkioTimeout oldDelegate = timeout.delegate();
    timeout.setDelegate(OkioTimeout.NONE);
  ...
  private final class FixedLengthSink implements OkioSink {
    private final OkioForwardingTimeout timeout = new OkioForwardingTimeout(sink.timeout());
   ...
   */
  private final class ChunkedSink implements OkioSink {
    private final OkioForwardingTimeout timeout = new OkioForwardingTimeout(sink.timeout());
    private boolean closed;
  ...
  private abstract class AbstractSource implements OkioSource {
    protected final OkioForwardingTimeout timeout = new OkioForwardingTimeout(source.timeout());
    ...
    protected final void endOfInput(boolean reuseConnection, IOException e) throws IOException {
      ...
      if (streamAllocation != null) {
        streamAllocation.streamFinished(!reuseConnection, OkHttp1Codec.this, bytesRead, e);
      }
    }
  }
  ...
  private class FixedLengthSource extends AbstractSource {
    ...
    @Override public void close() throws IOException {
      ...
      if (bytesRemaining != 0 && !OkUtil.discard(this, DISCARD_STREAM_TIMEOUT_MILLIS, MILLISECONDS)) {
  ...
  private class ChunkedSource extends AbstractSource {
    ...
    private final OkHttpUrl url;
    private long bytesRemainingInChunk = NO_CHUNK_YET;
    private boolean hasMoreChunks = true;

    ChunkedSource(OkHttpUrl url) {
      this.url = url;
    }
    ...
    private void readChunkSize() throws IOException {
      ...
      if (bytesRemainingInChunk == 0L) {
        hasMoreChunks = false;
        OkHttpHeaders.receiveHeaders(client.cookieJar(), url, readHeaders());
        endOfInput(true, null);
      }
    }
    ...
    @Override public void close() throws IOException {
      if (closed) return;
      if (hasMoreChunks && !OkUtil.discard(this, DISCARD_STREAM_TIMEOUT_MILLIS, MILLISECONDS)) {
        endOfInput(false, null);
      }
      closed = true;
    }
