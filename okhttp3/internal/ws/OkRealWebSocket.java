...
import static okhttp3.internal.ws.OkWebSocketProtocol.CLOSE_CLIENT_GOING_AWAY;
import static okhttp3.internal.ws.OkWebSocketProtocol.CLOSE_MESSAGE_MAX;
import static okhttp3.internal.ws.OkWebSocketProtocol.OPCODE_BINARY;
import static okhttp3.internal.ws.OkWebSocketProtocol.OPCODE_TEXT;
import static okhttp3.internal.ws.OkWebSocketProtocol.validateCloseCode;

import com.google.j2objc.annotations.WeakOuter;

public final class OkRealWebSocket implements OkWebSocket, OkWebSocketReader.FrameCallback {
  private static final List<OkProtocol> ONLY_HTTP1 = Collections.singletonList(OkProtocol.HTTP_1_1);
  ...
  private final OkRequest originalRequest;

  final OkWebSocketListener listener;
  ...
  private OkWebSocketReader reader;
  ...
  private OkWebSocketWriter writer;
  ...
  private final ArrayDeque<OkioByteString> pongQueue = new ArrayDeque<>();
  ...
  public OkRealWebSocket(OkRequest request, OkWebSocketListener listener, Random random,
    ...
    this.key = OkioByteString.of(nonce).base64();
  ...
  public void connect(OkHttpClient client) {
    client = client.newBuilder()
        .eventListener(OkEventListener.NONE)
        .protocols(ONLY_HTTP1)
        .build();
    final OkRequest request = originalRequest.newBuilder()
        .header("Upgrade", "websocket")
        .header("Connection", "Upgrade")
        .header("Sec-WebSocket-Key", key)
        .header("Sec-WebSocket-Version", "13")
        .build();
    call = OkInternal.instance.newWebSocketCall(client, request);
    call.enqueue(new OkCallback() {
      @Override public void onResponse(OkCall call, OkResponse response) {
        try {
        ...
        OkStreamAllocation streamAllocation = OkInternal.instance.streamAllocation(call);
        streamAllocation.noNewStreams(); // Prevent connection pooling!
        Streams streams = streamAllocation.connection().newWebSocketStreams(streamAllocation);

        // Process all web socket messages.
        try {
          listener.onOpen(OkRealWebSocket.this, response);
      ...
      @Override public void onFailure(OkCall call, IOException e) {
        failWebSocket(e, null);
      }
    });
  }

  void checkResponse(OkResponse response) throws ProtocolException {
    ...
    String acceptExpected = OkioByteString.encodeUtf8(key + OkWebSocketProtocol.ACCEPT_MAGIC)
        .sha1().base64();
  ...
  public void initReaderAndWriter(String name, Streams streams) throws IOException {
    synchronized (this) {
      ...
      this.writer = new OkWebSocketWriter(streams.client, streams.sink, random);
      this.executor = new ScheduledThreadPoolExecutor(1, OkUtil.threadFactory(name, false));
    ...
    reader = new OkWebSocketReader(streams.client, streams.source, this);
  }
  ...
  @Override public void onReadMessage(OkioByteString bytes) throws IOException {
    listener.onMessage(this, bytes);
  }

  @Override public synchronized void onReadPing(OkioByteString payload) {
  ...
  @Override public synchronized void onReadPong(OkioByteString buffer) {
  ...
  @Override public boolean send(String text) {
    ...
    return send(OkioByteString.encodeUtf8(text), OPCODE_TEXT);
  }

  @Override public boolean send(OkioByteString bytes) {
    if (bytes == null) throw new NullPointerException("bytes == null");
    return send(bytes, OPCODE_BINARY);
  }

  private synchronized boolean send(OkioByteString data, int formatOpcode) {
  ...
  synchronized boolean pong(OkioByteString payload) {
  ...
  synchronized boolean close(int code, String reason, long cancelAfterCloseMillis) {
    ...
    OkioByteString reasonBytes = null;
    if (reason != null) {
      reasonBytes = OkioByteString.encodeUtf8(reason);
  ...
  boolean writeOneFrame() throws IOException {
    ...
    synchronized (OkRealWebSocket.this) {
      if (failed) {
    ...
    try {
      if (pong != null) {
        ...
        OkioByteString data = ((Message) messageOrClose).data;
        OkioBufferedSink sink = Okio.buffer(writer.newMessageSink(
  ...
  void writePingFrame() {
    ...
    try {
      writer.writePing(OkioByteString.EMPTY);
    } catch (IOException e) {
      failWebSocket(e, null);
    }
  }

  public void failWebSocket(Exception e, OkResponse response) {
  ...
  static final class Message {
    ...
    final OkioByteString data;

    Message(int formatOpcode, OkioByteString data) {
  ...
  static final class Close {
    final int code;
    final OkioByteString reason;
    final long cancelAfterCloseMillis;

    Close(int code, OkioByteString reason, long cancelAfterCloseMillis) {
      this.code = code;
      this.reason = reason;
  ...
  public abstract static class Streams implements Closeable {
    ...
    public final OkioBufferedSource source;
    public final OkioBufferedSink sink;

    public Streams(boolean client, OkioBufferedSource source, OkioBufferedSink sink) {
      this.client = client;
      this.source = source;
      this.sink = sink;
    }
