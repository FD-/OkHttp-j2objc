 ...
 */
public final class OkConnectionSpecSelector {

  private final List<OkConnectionSpec> connectionSpecs;
  private int nextModeIndex;
  private boolean isFallbackPossible;
  private boolean isFallback;

  public OkConnectionSpecSelector(List<OkConnectionSpec> connectionSpecs) {
   ...
   * {@link OkConnectionSpec}. Returns the chosen {@link OkConnectionSpec}, never {@code null}.
   *
   * @throws IOException if the socket does not support any of the TLS modes available
   */
  public OkConnectionSpec configureSecureSocket(SSLSocket sslSocket) throws IOException {
    OkConnectionSpec tlsConfiguration = null;
    for (int i = nextModeIndex, size = connectionSpecs.size(); i < size; i++) {
      OkConnectionSpec connectionSpec = connectionSpecs.get(i);
    ...
    OkInternal.instance.apply(tlsConfiguration, sslSocket, isFallback);

    return tlsConfiguration;
  }

  /**
   * Reports a failure to complete a connection. Determines the next {@link OkConnectionSpec} to try,
   * if any.
   *
  ...
  /**
   * Returns {@code true} if any later {@link OkConnectionSpec} in the fallback strategy looks
