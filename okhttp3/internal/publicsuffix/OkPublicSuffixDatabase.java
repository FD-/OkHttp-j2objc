 ...
 */
public final class OkPublicSuffixDatabase {
  ...
  private static final OkPublicSuffixDatabase instance = new OkPublicSuffixDatabase();
  ...
  public static OkPublicSuffixDatabase get() {
    return instance;
  }
  ...
  private String[] findMatchingRule(String[] domainLabels) {
    ...
    for (int i = 0; i < domainLabels.length; i++) {
      domainLabelsUtf8Bytes[i] = domainLabels[i].getBytes(OkUtil.UTF_8);
    }
    ...
    while (low < high) {
      ...
      if (compareResult < 0) {
        ...
        if (labelBytesLeft < publicSuffixBytesLeft) {
          ...
          // Found a match.
          match = new String(bytesToSearch, mid, publicSuffixLength, OkUtil.UTF_8);
          break;
        }
      }
    }
    ...
    try {
      while (true) {
        try {
          ...
          OkPlatform.get().log(OkPlatform.WARN, "Failed to read public suffix list", e);
          return;
        }
      }
    } finally {
  ...
  private void readTheList() throws IOException {
    ...
    InputStream resource = OkPublicSuffixDatabase.class.getResourceAsStream(PUBLIC_SUFFIX_RESOURCE);
    if (resource == null) return;

    OkioBufferedSource bufferedSource = Okio.buffer(new OkioGzipSource(Okio.source(resource)));
    try {
