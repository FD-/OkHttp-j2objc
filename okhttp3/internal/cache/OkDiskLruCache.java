...
public final class OkDiskLruCache implements Closeable, Flushable {
  ...
  private long size = 0;
  OkioBufferedSink journalWriter;
  ...
  private final Runnable cleanupRunnable = new Runnable() {
    public void run() {
      synchronized (OkDiskLruCache.this) {
  ...
  OkDiskLruCache(OkFileSystem fileSystem, File directory, int appVersion, int valueCount, long maxSize,
                 Executor executor) {
  ...
  public synchronized void initialize() throws IOException {
    ...
    if (fileSystem.exists(journalFile)) {
      try {
        ...
        OkPlatform.get().log(WARN, "DiskLruCache " + directory + " is corrupt: "
   ...
   */
  public static OkDiskLruCache create(OkFileSystem fileSystem, File directory, int appVersion,
    ...
    Executor executor = new ThreadPoolExecutor(0, 1, 60L, TimeUnit.SECONDS,
        new LinkedBlockingQueue<Runnable>(), OkUtil.threadFactory("OkHttp DiskLruCache", true));

    return new OkDiskLruCache(fileSystem, directory, appVersion, valueCount, maxSize, executor);
  }

  private void readJournal() throws IOException {
    OkioBufferedSource source = Okio.buffer(fileSystem.source(journalFile));
    try {
      ...
      OkUtil.closeQuietly(source);
    }
  }

  private OkioBufferedSink newJournalWriter() throws FileNotFoundException {
    OkioSink fileSink = fileSystem.appendingSink(journalFile);
    OkioSink faultHidingSink = new OkFaultHidingSink(fileSink) {
      @Override protected void onException(IOException e) {
        assert (Thread.holdsLock(OkDiskLruCache.this));
  ...
  synchronized void rebuildJournal() throws IOException {
    ...
    OkioBufferedSink writer = Okio.buffer(fileSystem.sink(journalFileTmp));
    try {
  ...
  public synchronized Iterator<Snapshot> snapshots() throws IOException {
    ...
    return new Iterator<Snapshot>() {
      ...
      @Override public void remove() {
        ...
        try {
          OkDiskLruCache.this.remove(removeSnapshot.key);
        } catch (IOException ignored) {
  ...
  public final class Snapshot implements Closeable {
    ...
    private final OkioSource[] sources;
    private final long[] lengths;

    Snapshot(String key, long sequenceNumber, OkioSource[] sources, long[] lengths) {
      this.key = key;
    ...
    public Editor edit() throws IOException {
      return OkDiskLruCache.this.edit(key, sequenceNumber);
    }

    /** Returns the unbuffered stream with the value for {@code index}. */
    public OkioSource getSource(int index) {
    ...
    public void close() {
      for (OkioSource in : sources) {
        OkUtil.closeQuietly(in);
      }
    }
  }
  ...
  public final class Editor {
     ...
     * been committed.
     */
    public OkioSource newSource(int index) {
      synchronized (OkDiskLruCache.this) {
        if (done) {
          throw new IllegalStateException();
        }
     ...
     */
    public OkioSink newSink(int index) {
      synchronized (OkDiskLruCache.this) {
        if (done) {
          throw new IllegalStateException();
        }
        ...
        return new OkFaultHidingSink(sink) {
          @Override protected void onException(IOException e) {
            synchronized (OkDiskLruCache.this) {
              detach();
            }
  ...
  private final class Entry {
    ...
    void writeLengths(OkioBufferedSink writer) throws IOException {
     ...
     */
    Snapshot snapshot() {
      if (!Thread.holdsLock(OkDiskLruCache.this)) throw new AssertionError();

      OkioSource[] sources = new OkioSource[valueCount];
