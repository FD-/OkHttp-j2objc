...
import static okhttp3.internal.http2.OkHttp2.FLAG_NONE;
...
import static okhttp3.internal.http2.OkHttp2.illegalArgument;

/** Writes HTTP/2 transport frames. */
final class OkHttp2Writer implements Closeable {
  private static final Logger logger = Logger.getLogger(OkHttp2.class.getName());

  private final OkioBufferedSink sink;
  private final boolean client;
  private final OkioBuffer hpackBuffer;
  private int maxFrameSize;
  private boolean closed;

  final OkHpack.Writer hpackWriter;

  OkHttp2Writer(OkioBufferedSink sink, boolean client) {
    this.sink = sink;
    this.client = client;
    this.hpackBuffer = new OkioBuffer();
    this.hpackWriter = new OkHpack.Writer(hpackBuffer);
  ...
  public synchronized void applyAndAckSettings(OkSettings peerSettings) throws IOException {
    if (closed) throw new IOException("closed");
  ...
  public synchronized void pushPromise(int streamId, int promisedStreamId,
      List<OkHeader> requestHeaders) throws IOException {
    if (closed) throw new IOException("closed");
  ...
  public synchronized void synStream(boolean outFinished, int streamId,
      int associatedStreamId, List<OkHeader> headerBlock) throws IOException {
    if (closed) throw new IOException("closed");
    headers(outFinished, streamId, headerBlock);
  }

  public synchronized void synReply(boolean outFinished, int streamId,
      List<OkHeader> headerBlock) throws IOException {
    if (closed) throw new IOException("closed");
    headers(outFinished, streamId, headerBlock);
  }

  public synchronized void headers(int streamId, List<OkHeader> headerBlock)
      throws IOException {
    if (closed) throw new IOException("closed");
    headers(false, streamId, headerBlock);
  }

  public synchronized void rstStream(int streamId, OkErrorCode errorCode)
      throws IOException {
    if (closed) throw new IOException("closed");
   ...
   * #maxDataLength}.
   */
  public synchronized void data(boolean outFinished, int streamId, OkioBuffer source, int byteCount)
      throws IOException {
    if (closed) throw new IOException("closed");
    byte flags = FLAG_NONE;
  ...
  void dataFrame(int streamId, byte flags, OkioBuffer buffer, int byteCount) throws IOException {
    byte type = TYPE_DATA;
  ...
  public synchronized void settings(OkSettings settings) throws IOException {
    if (closed) throw new IOException("closed");
    ...
    int streamId = 0;
    frameHeader(streamId, length, type, flags);
    for (int i = 0; i < OkSettings.COUNT; i++) {
   ...
   */
  public synchronized void goAway(int lastGoodStreamId, OkErrorCode errorCode, byte[] debugData)
      throws IOException {
    if (closed) throw new IOException("closed");
  ...
  private static void writeMedium(OkioBufferedSink sink, int i) throws IOException {
  ...
  void headers(boolean outFinished, int streamId, List<OkHeader> headerBlock) throws IOException {
    if (closed) throw new IOException("closed");
