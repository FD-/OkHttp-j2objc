...
final class OkHpack {
  ...
  static final OkHeader[] STATIC_HEADER_TABLE = new OkHeader[] {
      new OkHeader(OkHeader.TARGET_AUTHORITY, ""),
      new OkHeader(OkHeader.TARGET_METHOD, "GET"),
      new OkHeader(OkHeader.TARGET_METHOD, "POST"),
      new OkHeader(OkHeader.TARGET_PATH, "/"),
      new OkHeader(OkHeader.TARGET_PATH, "/index.html"),
      new OkHeader(OkHeader.TARGET_SCHEME, "http"),
      new OkHeader(OkHeader.TARGET_SCHEME, "https"),
      new OkHeader(OkHeader.RESPONSE_STATUS, "200"),
      new OkHeader(OkHeader.RESPONSE_STATUS, "204"),
      new OkHeader(OkHeader.RESPONSE_STATUS, "206"),
      new OkHeader(OkHeader.RESPONSE_STATUS, "304"),
      new OkHeader(OkHeader.RESPONSE_STATUS, "400"),
      new OkHeader(OkHeader.RESPONSE_STATUS, "404"),
      new OkHeader(OkHeader.RESPONSE_STATUS, "500"),
      new OkHeader("accept-charset", ""),
      new OkHeader("accept-encoding", "gzip, deflate"),
      new OkHeader("accept-language", ""),
      new OkHeader("accept-ranges", ""),
      new OkHeader("accept", ""),
      new OkHeader("access-control-allow-origin", ""),
      new OkHeader("age", ""),
      new OkHeader("allow", ""),
      new OkHeader("authorization", ""),
      new OkHeader("cache-control", ""),
      new OkHeader("content-disposition", ""),
      new OkHeader("content-encoding", ""),
      new OkHeader("content-language", ""),
      new OkHeader("content-length", ""),
      new OkHeader("content-location", ""),
      new OkHeader("content-range", ""),
      new OkHeader("content-type", ""),
      new OkHeader("cookie", ""),
      new OkHeader("date", ""),
      new OkHeader("etag", ""),
      new OkHeader("expect", ""),
      new OkHeader("expires", ""),
      new OkHeader("from", ""),
      new OkHeader("host", ""),
      new OkHeader("if-match", ""),
      new OkHeader("if-modified-since", ""),
      new OkHeader("if-none-match", ""),
      new OkHeader("if-range", ""),
      new OkHeader("if-unmodified-since", ""),
      new OkHeader("last-modified", ""),
      new OkHeader("link", ""),
      new OkHeader("location", ""),
      new OkHeader("max-forwards", ""),
      new OkHeader("proxy-authenticate", ""),
      new OkHeader("proxy-authorization", ""),
      new OkHeader("range", ""),
      new OkHeader("referer", ""),
      new OkHeader("refresh", ""),
      new OkHeader("retry-after", ""),
      new OkHeader("server", ""),
      new OkHeader("set-cookie", ""),
      new OkHeader("strict-transport-security", ""),
      new OkHeader("transfer-encoding", ""),
      new OkHeader("user-agent", ""),
      new OkHeader("vary", ""),
      new OkHeader("via", ""),
      new OkHeader("www-authenticate", "")
  };
  ...
  static final class Reader {
    ...
    private final List<OkHeader> headerList = new ArrayList<>();
    ...
    // Visible for testing.
    OkHeader[] dynamicTable = new OkHeader[8];
    // Array is populated back to front, so new entries always have lowest index.
    int nextHeaderIndex = dynamicTable.length - 1;
    int headerCount = 0;
    int dynamicTableByteCount = 0;

    Reader(int headerTableSizeSetting, OkioSource source) {
      this(headerTableSizeSetting, headerTableSizeSetting, source);
    }

    Reader(int headerTableSizeSetting, int maxDynamicTableByteCount, OkioSource source) {
    ...
    public List<OkHeader> getAndResetHeaderList() {
      List<OkHeader> result = new ArrayList<>(headerList);
      headerList.clear();
      return result;
    }
    ...
    private void readIndexedHeader(int index) throws IOException {
      if (isStaticHeader(index)) {
        OkHeader staticEntry = STATIC_HEADER_TABLE[index];
    ...
    private void readLiteralHeaderWithoutIndexingIndexedName(int index) throws IOException {
      OkioByteString name = getName(index);
      OkioByteString value = readByteString();
      headerList.add(new OkHeader(name, value));
    }

    private void readLiteralHeaderWithoutIndexingNewName() throws IOException {
      OkioByteString name = checkLowercase(readByteString());
      OkioByteString value = readByteString();
      headerList.add(new OkHeader(name, value));
    }
    ...
    private void readLiteralHeaderWithIncrementalIndexingIndexedName(int nameIndex)
        throws IOException {
      OkioByteString name = getName(nameIndex);
      OkioByteString value = readByteString();
      insertIntoDynamicTable(-1, new OkHeader(name, value));
    }
    ...
    private OkioByteString getName(int index) throws IOException {
      if (isStaticHeader(index)) {
    ...
    /** index == -1 when new. */
    private void insertIntoDynamicTable(int index, OkHeader entry) {
      ...
      if (index == -1) { // Adding a value to the dynamic table.
        if (headerCount + 1 > dynamicTable.length) { // Need to grow the dynamic table.
          OkHeader[] doubled = new OkHeader[dynamicTable.length * 2];
    ...
    OkioByteString readByteString() throws IOException {
      int firstByte = readByte();
      boolean huffmanDecode = (firstByte & 0x80) == 0x80; // 1NNNNNNN
      int length = readInt(firstByte, PREFIX_7_BITS);

      if (huffmanDecode) {
        return OkioByteString.of(OkHuffman.get().decode(source.readByteArray(length)));
      } else {
  ...
  static final Map<OkioByteString, Integer> NAME_TO_FIRST_INDEX = nameToFirstIndex();

  private static Map<OkioByteString, Integer> nameToFirstIndex() {
    Map<OkioByteString, Integer> result = new LinkedHashMap<>(STATIC_HEADER_TABLE.length);
  ...
  static final class Writer {
    ...
    Writer(OkioBuffer out) {
      this(SETTINGS_HEADER_TABLE_SIZE, true, out);
    }

    Writer(int headerTableSizeSetting, boolean useCompression, OkioBuffer out) {
      this.headerTableSizeSetting = headerTableSizeSetting;
    ...
    private void insertIntoDynamicTable(OkHeader entry) {
      int delta = entry.hpackSize;
    ...
    void writeHeaders(List<OkHeader> headerBlock) throws IOException {
      ...
      for (int i = 0, size = headerBlock.size(); i < size; i++) {
        ...
        OkioByteString name = header.name.toAsciiLowercase();
        OkioByteString value = header.value;
        int headerIndex = -1;
        ...
        if (staticIndex != null) {
          ...
          if (headerNameIndex > 1 && headerNameIndex < 8) {
            ...
            if (OkUtil.equal(STATIC_HEADER_TABLE[headerNameIndex - 1].value, value)) {
              headerIndex = headerNameIndex;
            } else if (OkUtil.equal(STATIC_HEADER_TABLE[headerNameIndex].value, value)) {
        ...
        if (headerIndex == -1) {
          for (int j = nextHeaderIndex + 1, length = dynamicTable.length; j < length; j++) {
            if (OkUtil.equal(dynamicTable[j].name, name)) {
              if (OkUtil.equal(dynamicTable[j].value, value)) {
        ...
        } else if (name.startsWith(OkHeader.PSEUDO_PREFIX) && !OkHeader.TARGET_AUTHORITY.equals(name)) {
    ...
    void writeByteString(OkioByteString data) throws IOException {
      if (useCompression && OkHuffman.get().encodedLength(data) < data.size()) {
        OkioBuffer huffmanBuffer = new OkioBuffer();
        OkHuffman.get().encode(data, huffmanBuffer);
   ...
   * malformed.
   */
  static OkioByteString checkLowercase(OkioByteString name) throws IOException {
