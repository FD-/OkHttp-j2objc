...
public final class OkHttp2Stream {
  ...
  final int id;
  final OkHttp2Connection connection;

  /** Request headers. Immutable and non null. */
  private final List<OkHeader> requestHeaders;

  /** Response headers yet to be {@linkplain #takeResponseHeaders taken}. */
  private List<OkHeader> responseHeaders;
   ...
   */
  OkErrorCode errorCode = null;

  OkHttp2Stream(int id, OkHttp2Connection connection, boolean outFinished, boolean inFinished,
                List<OkHeader> requestHeaders) {
  ...
  public OkHttp2Connection getConnection() {
    return connection;
  }

  public List<OkHeader> getRequestHeaders() {
   ...
   * delimited by 'null'.
   */
  public synchronized List<OkHeader> takeResponseHeaders() throws IOException {
    ...
    List<OkHeader> result = responseHeaders;
    if (result != null) {
      responseHeaders = null;
      return result;
    }
    throw new OkStreamResetException(errorCode);
  }
   ...
   * been closed.
   */
  public synchronized OkErrorCode getErrorCode() {
    return errorCode;
  }
   ...
   */
  public void sendResponseHeaders(List<OkHeader> responseHeaders, boolean out) throws IOException {
    assert (!Thread.holdsLock(OkHttp2Stream.this));
  ...
  public OkioTimeout readTimeout() {
    return readTimeout;
  }

  public OkioTimeout writeTimeout() {
    return writeTimeout;
  }

  /** Returns a source that reads data from the peer. */
  public OkioSource getSource() {
    return source;
  }
   ...
   * transmitted.
   */
  public void close(OkErrorCode rstStatusCode) throws IOException {
   ...
   * immediately.
   */
  public void closeLater(OkErrorCode errorCode) {
  ...
  private boolean closeInternal(OkErrorCode errorCode) {
  ...
  void receiveHeaders(List<OkHeader> headers) {
    assert (!Thread.holdsLock(OkHttp2Stream.this));
    boolean open = true;
    synchronized (this) {
      ...
      if (responseHeaders == null) {
        ...
        List<OkHeader> newHeaders = new ArrayList<>();
  ...
  void receiveData(OkioBufferedSource in, int length) throws IOException {
    assert (!Thread.holdsLock(OkHttp2Stream.this));
  ...
  synchronized void receiveRstStream(OkErrorCode errorCode) {
    if (this.errorCode == null) {
      this.errorCode = errorCode;
      notifyAll();
    }
   ...
   */
  @WeakOuter
  private final class FramingSource implements OkioSource {
    /** Buffer to receive data from the network into. Only accessed by the reader thread. */
    private final OkioBuffer receiveBuffer = new OkioBuffer();

    /** Buffer with readable data. Guarded by Http2Stream.this. */
    private final OkioBuffer readBuffer = new OkioBuffer();
    ...
    @Override public long read(OkioBuffer sink, long byteCount) throws IOException {
      ...
      long read = -1;
      OkErrorCode errorCode;
      synchronized (OkHttp2Stream.this) {
        waitUntilReadable();
        if (closed) {
          throw new IOException("stream closed");
        }
        errorCode = OkHttp2Stream.this.errorCode;
    ...
    void receive(OkioBufferedSource in, long byteCount) throws IOException {
      assert (!Thread.holdsLock(OkHttp2Stream.this));
      ...
      while (byteCount > 0) {
        ...
        if (flowControlError) {
          in.skip(byteCount);
          closeLater(OkErrorCode.FLOW_CONTROL_ERROR);
          return;
        }
  ...
  void cancelStreamIfNecessary() throws IOException {
    ...
    if (cancel) {
      ...
      OkHttp2Stream.this.close(OkErrorCode.CANCEL);
    } else if (!open) {
      connection.removeStream(id);
    }
  }
  ...
  @WeakOuter
  final class FramingSink implements OkioSink {
     ...
     */
    private final OkioBuffer sendBuffer = new OkioBuffer();
  ...
  void addBytesToWriteWindow(long delta) {
    ...
    if (delta > 0) OkHttp2Stream.this.notifyAll();
  }
   ...
   */
  @WeakOuter
  class StreamTimeout extends OkioAsyncTimeout {
    @Override protected void timedOut() {
      closeLater(OkErrorCode.CANCEL);
    }
