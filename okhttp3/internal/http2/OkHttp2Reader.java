...
import static okhttp3.internal.OkUtil.format;
import static okhttp3.internal.http2.OkHttp2.CONNECTION_PREFACE;
import static okhttp3.internal.http2.OkHttp2.FLAG_ACK;
import static okhttp3.internal.http2.OkHttp2.FLAG_COMPRESSED;
import static okhttp3.internal.http2.OkHttp2.FLAG_END_HEADERS;
import static okhttp3.internal.http2.OkHttp2.FLAG_END_STREAM;
import static okhttp3.internal.http2.OkHttp2.FLAG_PADDED;
import static okhttp3.internal.http2.OkHttp2.FLAG_PRIORITY;
import static okhttp3.internal.http2.OkHttp2.INITIAL_MAX_FRAME_SIZE;
import static okhttp3.internal.http2.OkHttp2.TYPE_CONTINUATION;
import static okhttp3.internal.http2.OkHttp2.TYPE_DATA;
import static okhttp3.internal.http2.OkHttp2.TYPE_GOAWAY;
import static okhttp3.internal.http2.OkHttp2.TYPE_HEADERS;
import static okhttp3.internal.http2.OkHttp2.TYPE_PING;
import static okhttp3.internal.http2.OkHttp2.TYPE_PRIORITY;
import static okhttp3.internal.http2.OkHttp2.TYPE_PUSH_PROMISE;
import static okhttp3.internal.http2.OkHttp2.TYPE_RST_STREAM;
import static okhttp3.internal.http2.OkHttp2.TYPE_SETTINGS;
import static okhttp3.internal.http2.OkHttp2.TYPE_WINDOW_UPDATE;
import static okhttp3.internal.http2.OkHttp2.frameLog;
import static okhttp3.internal.http2.OkHttp2.ioException;
import static okio.OkioByteString.EMPTY;

/**
 * Reads HTTP/2 transport frames.
 *
 * <p>This implementation assumes we do not send an increased {@link OkSettings#getMaxFrameSize frame
 * size setting} to the peer. Hence, we expect all frames to have a max length of {@link
 * OkHttp2#INITIAL_MAX_FRAME_SIZE}.
 */
final class OkHttp2Reader implements Closeable {
  static final Logger logger = Logger.getLogger(OkHttp2.class.getName());
  ...
  // Visible for testing.
  final OkHpack.Reader hpackReader;

  /** Creates a frame reader with max header table size of 4096. */
  OkHttp2Reader(OkioBufferedSource source, boolean client) {
    this.source = source;
    this.client = client;
    this.continuation = new ContinuationSource(this.source);
    this.hpackReader = new OkHpack.Reader(4096, continuation);
  }
  ...
  public void readConnectionPreface(Handler handler) throws IOException {
    if (client) {
      ...
      OkioByteString connectionPreface = source.readByteString(CONNECTION_PREFACE.size());
  ...
  private void readHeaders(Handler handler, int length, byte flags, int streamId)
    ...
    List<OkHeader> headerBlock = readHeaderBlock(length, padding, flags, streamId);

    handler.headers(endStream, streamId, -1, headerBlock);
  }

  private List<OkHeader> readHeaderBlock(int length, short padding, byte flags, int streamId)
      throws IOException {
  ...
  private void readRstStream(Handler handler, int length, byte flags, int streamId)
    ...
    OkErrorCode errorCode = OkErrorCode.fromHttp2(errorCodeInt);
    if (errorCode == null) {
  ...
  private void readSettings(Handler handler, int length, byte flags, int streamId)
    ...
    OkSettings settings = new OkSettings();
  ...
  private void readGoAway(Handler handler, int length, byte flags, int streamId)
    ...
    OkioByteString debugData = EMPTY;
   ...
   * continuation frames as they are needed by {@link OkHpack.Reader#readHeaders()}.
   */
  static final class ContinuationSource implements OkioSource {
    private final OkioBufferedSource source;
    ...
    ContinuationSource(OkioBufferedSource source) {
      this.source = source;
    }
  ...
  static int readMedium(OkioBufferedSource source) throws IOException {
  ...
  interface Handler {
    void data(boolean inFinished, int streamId, OkioBufferedSource source, int length)
        throws IOException;
    ...
    void rstStream(int streamId, OkErrorCode errorCode);

    void settings(boolean clearPrevious, OkSettings settings);
     ...
     */
    void goAway(int lastGoodStreamId, OkErrorCode errorCode, OkioByteString debugData);
     ...
     */
    void pushPromise(int streamId, int promisedStreamId, List<OkHeader> requestHeaders)
        throws IOException;
     ...
     */
    void alternateService(int streamId, String origin, OkioByteString protocol, String host, int port,
                          long maxAge);
  }
}
