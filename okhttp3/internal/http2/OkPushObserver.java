...
/**
 * {@link OkProtocol#HTTP_2 HTTP/2} only. Processes server-initiated HTTP requests on the client.
 ...
 */
public interface OkPushObserver {
  /**
   ...
   * and {@code :path}.
   */
  boolean onRequest(int streamId, List<OkHeader> requestHeaders);
   ...
   */
  boolean onHeaders(int streamId, List<OkHeader> responseHeaders, boolean last);
   ...
   */
  boolean onData(int streamId, OkioBufferedSource source, int byteCount, boolean last)
      throws IOException;

  /** Indicates the reason why this stream was canceled. */
  void onReset(int streamId, OkErrorCode errorCode);

  OkPushObserver CANCEL = new OkPushObserver() {

    @Override public boolean onRequest(int streamId, List<OkHeader> requestHeaders) {
      return true;
    }

    @Override public boolean onHeaders(int streamId, List<OkHeader> responseHeaders, boolean last) {
      return true;
    }

    @Override public boolean onData(int streamId, OkioBufferedSource source, int byteCount,
                                    boolean last) throws IOException {
      source.skip(byteCount);
      return true;
    }

    @Override public void onReset(int streamId, OkErrorCode errorCode) {
    }
  };
}
