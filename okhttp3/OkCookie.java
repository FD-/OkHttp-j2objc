...
import okhttp3.internal.publicsuffix.OkPublicSuffixDatabase;

import static okhttp3.internal.OkUtil.UTC;
import static okhttp3.internal.OkUtil.canonicalizeHost;
import static okhttp3.internal.OkUtil.delimiterOffset;
import static okhttp3.internal.OkUtil.indexOfControlOrNonAscii;
import static okhttp3.internal.OkUtil.trimSubstring;
import static okhttp3.internal.OkUtil.verifyAsIpAddress;
...
public final class OkCookie {
  ...
  private OkCookie(String name, String value, long expiresAt, String domain, String path,
   ...
   */
  public boolean matches(OkHttpUrl url) {
  ...
  private static boolean pathMatch(OkHttpUrl url, String path) {
   ...
   */
  public static
  OkCookie parse(OkHttpUrl url, String setCookie) {
    return parse(System.currentTimeMillis(), url, setCookie);
  }

  static
  OkCookie parse(long currentTimeMillis, OkHttpUrl url, String setCookie) {
    int pos = 0;
    ...
    long expiresAt = OkHttpDate.MAX_DATE;
    ...
    if (deltaSeconds == Long.MIN_VALUE) {
      ...
      if (expiresAt < currentTimeMillis || expiresAt > OkHttpDate.MAX_DATE) {
        expiresAt = OkHttpDate.MAX_DATE; // Handle overflow & limit the date range.
      }
    }
    ...
    if (urlHost.length() != domain.length()
        && OkPublicSuffixDatabase.get().getEffectiveTldPlusOne(domain) == null) {
      return null;
    }
    ...
    return new OkCookie(cookieName, cookieValue, expiresAt, domain, path, secureOnly, httpOnly,
        hostOnly, persistent);
  }
  ...
  public static List<OkCookie> parseAll(OkHttpUrl url, OkHeaders headers) {
    List<String> cookieStrings = headers.values("Set-Cookie");
    List<OkCookie> cookies = null;

    for (int i = 0, size = cookieStrings.size(); i < size; i++) {
      OkCookie cookie = OkCookie.parse(url, cookieStrings.get(i));
    ...
    return cookies != null
        ...
        : Collections.<OkCookie>emptyList();
  }
  ...
  public static final class Builder {
    ...
    public Builder expiresAt(long expiresAt) {
      ...
      if (expiresAt > OkHttpDate.MAX_DATE) expiresAt = OkHttpDate.MAX_DATE;
      this.expiresAt = expiresAt;
    ...
    private Builder domain(String domain, boolean hostOnly) {
      ...
      String canonicalDomain = OkUtil.canonicalizeHost(domain);
      if (canonicalDomain == null) {
    ...
    public OkCookie build() {
      return new OkCookie(this);
    }
  }
  ...
  String toString(boolean forObsoleteRfc2965) {
    ...
    if (persistent) {
      if (expiresAt == Long.MIN_VALUE) {
        ...
        result.append("; expires=").append(OkHttpDate.format(new Date(expiresAt)));
      }
    }
  ...
  @Override public boolean equals(Object other) {
    if (!(other instanceof OkCookie)) return false;
    OkCookie that = (OkCookie) other;
