 ...
 */
public interface OkioBufferedSink extends OkioSink, WritableByteChannel {
  /** Returns this sink's internal buffer. */
  OkioBuffer buffer();

  OkioBufferedSink write(OkioByteString byteString) throws IOException;

  /**
   * Like {@link OutputStream#write(byte[])}, this writes a complete byte array to
   * this sink.
   */
  OkioBufferedSink write(byte[] source) throws IOException;

  /**
   * Like {@link OutputStream#write(byte[], int, int)}, this writes {@code byteCount}
   * bytes of {@code source}, starting at {@code offset}.
   */
  OkioBufferedSink write(byte[] source, int offset, int byteCount) throws IOException;
   ...
   */
  long writeAll(OkioSource source) throws IOException;

  /** Removes {@code byteCount} bytes from {@code source} and appends them to this sink. */
  OkioBufferedSink write(OkioSource source, long byteCount) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink writeUtf8(String string) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink writeUtf8(String string, int beginIndex, int endIndex) throws IOException;

  /** Encodes {@code codePoint} in UTF-8 and writes it to this sink. */
  OkioBufferedSink writeUtf8CodePoint(int codePoint) throws IOException;

  /** Encodes {@code string} in {@code charset} and writes it to this sink. */
  OkioBufferedSink writeString(String string, Charset charset) throws IOException;

  /**
   * Encodes the characters at {@code beginIndex} up to {@code endIndex} from {@code string} in
   * {@code charset} and writes it to this sink.
   */
  OkioBufferedSink writeString(String string, int beginIndex, int endIndex, Charset charset)
      throws IOException;

  /** Writes a byte to this sink. */
  OkioBufferedSink writeByte(int b) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink writeShort(int s) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink writeShortLe(int s) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink writeInt(int i) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink writeIntLe(int i) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink writeLong(long v) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink writeLongLe(long v) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink writeDecimalLong(long v) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink writeHexadecimalUnsignedLong(long v) throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink emit() throws IOException;
   ...
   * }</pre>
   */
  OkioBufferedSink emitCompleteSegments() throws IOException;
