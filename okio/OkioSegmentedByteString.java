 ...
 */
final class OkioSegmentedByteString extends OkioByteString {
  final transient byte[][] segments;
  final transient int[] directory;

  OkioSegmentedByteString(OkioBuffer buffer, int byteCount) {
    super(null);
    ...
    int offset = 0;
    int segmentCount = 0;
    for (OkioSegment s = buffer.head; offset < byteCount; s = s.next) {
      if (s.limit == s.pos) {
  ...
  @Override public OkioByteString toAsciiLowercase() {
    return toByteString().toAsciiLowercase();
  }

  @Override public OkioByteString toAsciiUppercase() {
    return toByteString().toAsciiUppercase();
  }

  @Override public OkioByteString md5() {
    return toByteString().md5();
  }

  @Override public OkioByteString sha1() {
    return toByteString().sha1();
  }

  @Override public OkioByteString sha256() {
    return toByteString().sha256();
  }

  @Override public OkioByteString hmacSha1(OkioByteString key) {
    return toByteString().hmacSha1(key);
  }

  @Override public OkioByteString hmacSha256(OkioByteString key) {
  ...
  @Override public OkioByteString substring(int beginIndex) {
    return toByteString().substring(beginIndex);
  }

  @Override public OkioByteString substring(int beginIndex, int endIndex) {
  ...
  @Override void write(OkioBuffer buffer) {
    int segmentOffset = 0;
    for (int s = 0, segmentCount = segments.length; s < segmentCount; s++) {
      int segmentPos = directory[segmentCount + s];
      int nextSegmentOffset = directory[s];
      OkioSegment segment = new OkioSegment(segments[s], segmentPos,
  ...
  @Override public boolean rangeEquals(
          int offset, OkioByteString other, int otherOffset, int byteCount) {
  ...
  private OkioByteString toByteString() {
    return new OkioByteString(toByteArray());
  }
  ...
  @Override public boolean equals(Object o) {
    ...
    return o instanceof OkioByteString
        && ((OkioByteString) o).size() == size()
        && rangeEquals(0, ((OkioByteString) o), 0, size());
  }
