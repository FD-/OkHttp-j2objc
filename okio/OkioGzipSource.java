 ...
 */
public final class OkioGzipSource implements OkioSource {
   ...
   */
  private final OkioInflaterSource inflaterSource;

  /** Checksum used to check both the GZIP header and decompressed body. */
  private final CRC32 crc = new CRC32();

  public OkioGzipSource(OkioSource source) {
    if (source == null) throw new IllegalArgumentException("source == null");
    this.inflater = new Inflater(true);
    this.source = Okio.buffer(source);
    this.inflaterSource = new OkioInflaterSource(this.source, inflater);
  }
  ...
  private void updateCrc(OkioBuffer buffer, long offset, long byteCount) {
    // Skip segments that we aren't checksumming.
    OkioSegment s = buffer.head;
