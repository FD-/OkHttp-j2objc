 ...
 */
public class OkioAsyncTimeout extends OkioTimeout {
  /**
  ...
  private static synchronized void scheduleTimeout(
          OkioAsyncTimeout node, long timeoutNanos, boolean hasDeadline) {
    // Start the watchdog thread and create the head node when the first timeout is scheduled.
    if (head == null) {
      head = new OkioAsyncTimeout();
      new Watchdog().start();
    }
    ...
    for (OkioAsyncTimeout prev = head; true; prev = prev.next) {
      if (prev.next == null || remainingNanos < prev.next.remainingNanos(now)) {
        node.next = prev.next;
        prev.next = node;
        if (prev == head) {
          OkioAsyncTimeout.class.notify(); // Wake up the watchdog when inserting at the front.
        }
        break;
      }
    }
  ...
  /** Returns true if the timeout occurred. */
  private static synchronized boolean cancelScheduledTimeout(OkioAsyncTimeout node) {
    // Remove the node from the linked list.
    for (OkioAsyncTimeout prev = head; prev != null; prev = prev.next) {
      if (prev.next == node) {
        prev.next = node.next;
        node.next = null;
        return false;
   ...
   */
  public final OkioSink sink(final OkioSink sink) {
    return new OkioSink() {
      @Override public void write(OkioBuffer source, long byteCount) throws IOException {
        ...
        while (byteCount > 0L) {
          ...
          long toWrite = 0L;
          for (OkioSegment s = source.head; toWrite < TIMEOUT_WRITE_SIZE; s = s.next) {
   ...
   */
  public final OkioSource source(final OkioSource source) {
    ...
    public void run() {
      while (true) {
        try {
          ...
          synchronized (OkioAsyncTimeout.class) {
   ...
   */
  static
  OkioAsyncTimeout awaitTimeout() throws InterruptedException {
    // Get the next eligible node.
    OkioAsyncTimeout node = head.next;

    // The queue is empty. Wait until either something is enqueued or the idle timeout elapses.
    if (node == null) {
      long startNanos = System.nanoTime();
      OkioAsyncTimeout.class.wait(IDLE_TIMEOUT_MILLIS);
    ...
    if (waitNanos > 0) {
      ...
      OkioAsyncTimeout.class.wait(waitMillis, (int) waitNanos);
      return null;
    }
