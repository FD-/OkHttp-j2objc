 ...
 */
public final class OkioBuffer implements OkioBufferedSource, OkioBufferedSink, Cloneable, ByteChannel {
  ...
  // Workaround for j2objc leak
  // Discussion at https://github.com/google/j2objc/issues/822
  ...
  @Override public OutputStream outputStream() {
    return new OutputStream() {
      ...
      @Override public void write(byte[] data, int offset, int byteCount) {
        OkioBuffer.this.write(data, offset, byteCount);
      }
      ...
      @Override public String toString() {
        return OkioBuffer.this + ".outputStream()";
      }
    };
  }

  @Override public OkioBuffer emitCompleteSegments() {
    return this; // Nowhere to emit to!
  }

  @Override public OkioBufferedSink emit() {
    return this; // Nowhere to emit to!
  }
  ...
  @Override public InputStream inputStream() {
    return new InputStream() {
      ...
      @Override public int read(byte[] sink, int offset, int byteCount) {
        return OkioBuffer.this.read(sink, offset, byteCount);
      }
      ...
      @Override public String toString() {
        return OkioBuffer.this + ".inputStream()";
      }
    };
  }

  /** Copy the contents of this to {@code out}. */
  public OkioBuffer copyTo(OutputStream out) throws IOException {
   ...
   * {@code out}.
   */
  public OkioBuffer copyTo(OutputStream out, long offset, long byteCount) throws IOException {
  ...
  public OkioBuffer copyTo(OkioBuffer out, long offset, long byteCount) {
    if (out == null) throw new IllegalArgumentException("out == null");
    checkOffsetAndCount(size, offset, byteCount);
    if (byteCount == 0) return this;
  ...
  public OkioBuffer writeTo(OutputStream out) throws IOException {
    return writeTo(out, size);
  }

  /** Write {@code byteCount} bytes from this to {@code out}. */
  public OkioBuffer writeTo(OutputStream out, long byteCount) throws IOException {
    if (out == null) throw new IllegalArgumentException("out == null");
    ...
    while (byteCount > 0) {
      ...
      if (s.pos == s.limit) {
        ...
        OkioSegmentPool.recycle(toRecycle);
      }
    }
  ...
  public OkioBuffer readFrom(InputStream in) throws IOException {
    readFrom(in, Long.MAX_VALUE, true);
    return this;
  }

  /** Read {@code byteCount} bytes from {@code in} to this. */
  public OkioBuffer readFrom(InputStream in, long byteCount) throws IOException {
  ...
  private void readFrom(InputStream in, long byteCount, boolean forever) throws IOException {
    ...
    while (byteCount > 0 || forever) {
      OkioSegment tail = writableSegment(1);
  ...
  public long completeSegmentByteCount() {
    ...
    OkioSegment tail = head.prev;
    if (tail.limit < OkioSegment.SIZE && tail.owner) {
  ...
  @Override public byte readByte() {
    ...
    if (pos == limit) {
      head = segment.pop();
      OkioSegmentPool.recycle(segment);
    } else {
      segment.pos = pos;
    }
  ...
  public byte getByte(long pos) {
    ...
    if (size - pos > pos) {
      for (OkioSegment s = head; true; s = s.next) {
      ...
      pos -= size;
      for (OkioSegment s = head.prev; true; s = s.prev) {
        pos += s.limit - s.pos;
      ...
      for (; pos < limit; pos++, seen++) {
        ...
        if (b >= '0' && b <= '9') {
          ...
          if (value < overflowZone || value == overflowZone && digit < overflowDigit) {
            OkioBuffer buffer = new OkioBuffer().writeDecimalLong(value).writeByte(b);
    ...
    do {
      ...
      for (; pos < limit; pos++, seen++) {
        ...
        if ((value & 0xf000000000000000L) != 0) {
          OkioBuffer buffer = new OkioBuffer().writeHexadecimalUnsignedLong(value).writeByte(b);
          throw new NumberFormatException("Number too large: " + buffer.readUtf8());
        }
  ...
  @Override public OkioByteString readByteString() {
    return new OkioByteString(readByteArray());
  }

  @Override public OkioByteString readByteString(long byteCount) throws EOFException {
    return new OkioByteString(readByteArray(byteCount));
  }

  @Override public int select(OkioOptions options) {
    OkioSegment s = head;
    if (s == null) return options.indexOf(OkioByteString.EMPTY);

    OkioByteString[] byteStrings = options.byteStrings;
    for (int i = 0, listSize = byteStrings.length; i < listSize; i++) {
      OkioByteString b = byteStrings[i];
   ...
   */
  int selectPrefix(OkioOptions options) {
    OkioSegment s = head;
    OkioByteString[] byteStrings = options.byteStrings;
    for (int i = 0, listSize = byteStrings.length; i < listSize; i++) {
      OkioByteString b = byteStrings[i];
  ...
  @Override public void readFully(OkioBuffer sink, long byteCount) throws EOFException {
    if (size < byteCount) {
  ...
  @Override public long readAll(OkioSink sink) throws IOException {
    long byteCount = size;
    if (byteCount > 0) {
      sink.write(this, byteCount);
    }
  ...
  @Override public String readUtf8LineStrict(long limit) throws EOFException {
    ...
    OkioBuffer data = new OkioBuffer();
  ...
  @Override public OkioBuffer write(OkioByteString byteString) {
    if (byteString == null) throw new IllegalArgumentException("byteString == null");
    byteString.write(this);
    return this;
  }

  @Override public OkioBuffer writeUtf8(String string) {
    return writeUtf8(string, 0, string.length());
  }

  @Override public OkioBuffer writeUtf8(String string, int beginIndex, int endIndex) {
    ...
    for (int i = beginIndex; i < endIndex;) {
      ...
      if (c < 0x80) {
        ...
        int runLimit = Math.min(endIndex, OkioSegment.SIZE - segmentOffset);
  ...
  @Override public OkioBuffer writeUtf8CodePoint(int codePoint) {
    if (codePoint < 0x80) {
  ...
  @Override public OkioBuffer writeString(String string, Charset charset) {
    return writeString(string, 0, string.length(), charset);
  }

  @Override
  public OkioBuffer writeString(String string, int beginIndex, int endIndex, Charset charset) {
    if (string == null) throw new IllegalArgumentException("string == null");
  ...
  @Override public OkioBuffer write(byte[] source) {
    if (source == null) throw new IllegalArgumentException("source == null");
    return write(source, 0, source.length);
  }

  @Override public OkioBuffer write(byte[] source, int offset, int byteCount) {
    if (source == null) throw new IllegalArgumentException("source == null");
    ...
    while (offset < limit) {
      ...
      int toCopy = Math.min(limit - offset, OkioSegment.SIZE - tail.limit);
  ...
  @Override public int write(ByteBuffer source) throws IOException {
    ...
    while (remaining > 0) {
      ...
      int toCopy = Math.min(remaining, OkioSegment.SIZE - tail.limit);
  ...
  @Override public long writeAll(OkioSource source) throws IOException {
    ...
    for (long readCount; (readCount = source.read(this, OkioSegment.SIZE)) != -1; ) {
  ...
  @Override public OkioBuffer writeByte(int b) {
    OkioSegment tail = writableSegment(1);
  ...
  @Override public OkioBuffer writeShort(int s) {
    OkioSegment tail = writableSegment(2);
    byte[] data = tail.data;
  ...
  @Override public OkioBuffer writeShortLe(int s) {
    return writeShort(OkioUtil.reverseBytesShort((short) s));
  }

  @Override public OkioBuffer writeInt(int i) {
    OkioSegment tail = writableSegment(4);
    byte[] data = tail.data;
    int limit = tail.limit;
  ...
  @Override public OkioBuffer writeIntLe(int i) {
    return writeInt(OkioUtil.reverseBytesInt(i));
  }

  @Override public OkioBuffer writeLong(long v) {
    OkioSegment tail = writableSegment(8);
    byte[] data = tail.data;
    int limit = tail.limit;
  ...
  @Override public OkioBuffer writeLongLe(long v) {
    return writeLong(reverseBytesLong(v));
  }

  @Override public OkioBuffer writeDecimalLong(long v) {
    if (v == 0) {
    ...
    OkioSegment tail = writableSegment(width);
    byte[] data = tail.data;
  ...
  @Override public OkioBuffer writeHexadecimalUnsignedLong(long v) {
    if (v == 0) {
      // Both a shortcut and required since the following code can't handle zero.
      return writeByte('0');
    }
   ...
   */
  OkioSegment writableSegment(int minimumCapacity) {
    if (minimumCapacity < 1 || minimumCapacity > OkioSegment.SIZE) throw new IllegalArgumentException();

    if (head == null) {
      head = OkioSegmentPool.take(); // Acquire a first segment.
      return head.next = head.prev = head;
    }

    OkioSegment tail = head.prev;
    if (tail.limit + minimumCapacity > OkioSegment.SIZE || !tail.owner) {
      tail = tail.push(OkioSegmentPool.take()); // Append a new empty segment to fill up.
    }
    return tail;
  }
  ...
  @Override public void write(OkioBuffer source, long byteCount) {
    ...
    while (byteCount > 0) {
      ...
      if (byteCount < (source.head.limit - source.head.pos)) {
        OkioSegment tail = head != null ? head.prev : null;
        if (tail != null && tail.owner
            && (byteCount + tail.limit - (tail.shared ? 0 : tail.pos) <= OkioSegment.SIZE)) {
      ...
      OkioSegment segmentToMove = source.head;
  ...
  @Override public long read(OkioBuffer sink, long byteCount) {
  ...
  @Override public long indexOf(OkioByteString bytes) throws IOException {
    return indexOf(bytes, 0);
  }

  @Override public long indexOf(OkioByteString bytes, long fromIndex) throws IOException {
  ...
  @Override public long indexOfElement(OkioByteString targetBytes) {
    return indexOfElement(targetBytes, 0);
  }

  @Override public long indexOfElement(OkioByteString targetBytes, long fromIndex) {
    if (fromIndex < 0) throw new IllegalArgumentException("fromIndex < 0");
  ...
  @Override public boolean rangeEquals(long offset, OkioByteString bytes) {
    return rangeEquals(offset, bytes, 0, bytes.size());
  }

  @Override public boolean rangeEquals(
          long offset, OkioByteString bytes, int bytesOffset, int byteCount) {
    if (offset < 0
        || bytesOffset < 0
        || byteCount < 0
  ...
  private boolean rangeEquals(
          OkioSegment segment, int segmentPos, OkioByteString bytes, int bytesOffset, int bytesLimit) {
  ...
  List<Integer> segmentSizes() {
    ...
    for (OkioSegment s = head.next; s != head; s = s.next) {
  ...
  public OkioByteString md5() {
    return digest("MD5");
  }

  /** Returns the 160-bit SHA-1 hash of this buffer. */
  public OkioByteString sha1() {
    return digest("SHA-1");
  }

  /** Returns the 256-bit SHA-256 hash of this buffer. */
  public OkioByteString sha256() {
    return digest("SHA-256");
  }

  /** Returns the 512-bit SHA-512 hash of this buffer. */
  public OkioByteString sha512() {
      return digest("SHA-512");
  }

  private OkioByteString digest(String algorithm) {
    try {
      ...
      return OkioByteString.of(messageDigest.digest());
  ...
  @Override public boolean equals(Object o) {
    ...
    if (!(o instanceof OkioBuffer)) return false;
    OkioBuffer that = (OkioBuffer) o;
    if (size != that.size) return false;
    if (size == 0) return true; // Both buffers are empty.

    OkioSegment sa = this.head;
    OkioSegment sb = that.head;
    int posA = sa.pos;
    int posB = sb.pos;
  ...
  @Override public OkioBuffer clone() {
    OkioBuffer result = new OkioBuffer();
  ...
  public OkioByteString snapshot() {
   ...
   */
  public OkioByteString snapshot(int byteCount) {
    if (byteCount == 0) return OkioByteString.EMPTY;
    return new OkioSegmentedByteString(this, byteCount);
  }
   ...
   *
   * <p>Use {@link OkioBuffer#readUnsafe} to create a cursor to read buffer data and {@link
   * OkioBuffer#readAndWriteUnsafe} to create a cursor to read and write buffer data. In either case,
   ...
   * read-only operations like {@link OkioBuffer#clone} are unsafe because they mark segments as shared.
   *
  ...
  public static final class UnsafeCursor implements Closeable {
    public OkioBuffer buffer;
    public boolean readWrite;

    private OkioSegment segment;
    ...
    public int seek(long offset) {
      ...
      OkioSegment head = buffer.head;
      OkioSegment tail = buffer.head;
      ...
      if (readWrite && next.shared) {
        OkioSegment unsharedNext = next.unsharedCopy();
        if (buffer.head == next) {
    ...
    public long resizeBuffer(long newSize) {
      ...
      if (newSize <= oldSize) {
        ...
        for (long bytesToAdd = newSize - oldSize; bytesToAdd > 0; ) {
          OkioSegment tail = buffer.writableSegment(1);
          int segmentBytesToAdd = (int) Math.min(bytesToAdd, OkioSegment.SIZE - tail.limit);
    ...
    public long expandBuffer(int minByteCount) {
      ...
      if (minByteCount > OkioSegment.SIZE) {
      ...
      long oldSize = buffer.size;
      OkioSegment tail = buffer.writableSegment(minByteCount);
      int result = OkioSegment.SIZE - tail.limit;
      tail.limit = OkioSegment.SIZE;
      ...
      this.offset = oldSize;
      this.data = tail.data;
      this.start = OkioSegment.SIZE - result;
      this.end = OkioSegment.SIZE;
