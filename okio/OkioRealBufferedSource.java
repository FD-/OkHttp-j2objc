...
final class OkioRealBufferedSource implements OkioBufferedSource {
  public final OkioBuffer buffer = new OkioBuffer();
  public final OkioSource source;
  boolean closed;

  OkioRealBufferedSource(OkioSource source) {
  ...
  @Override public long read(OkioBuffer sink, long byteCount) throws IOException {
    ...
    if (buffer.size == 0) {
      long read = source.read(buffer, OkioSegment.SIZE);
      if (read == -1) return -1;
    }
  ...
  @Override public boolean exhausted() throws IOException {
    if (closed) throw new IllegalStateException("closed");
    return buffer.exhausted() && source.read(buffer, OkioSegment.SIZE) == -1;
  }
  ...
  @Override public boolean request(long byteCount) throws IOException {
    ...
    while (buffer.size < byteCount) {
      if (source.read(buffer, OkioSegment.SIZE) == -1) return false;
    }
    return true;
  }
  ...
  @Override public OkioByteString readByteString() throws IOException {
    buffer.writeAll(source);
    return buffer.readByteString();
  }

  @Override public OkioByteString readByteString(long byteCount) throws IOException {
    require(byteCount);
    return buffer.readByteString(byteCount);
  }

  @Override public int select(OkioOptions options) throws IOException {
    if (closed) throw new IllegalStateException("closed");
    ...
    while (true) {
      ...
      if (source.read(buffer, OkioSegment.SIZE) == -1) return -1;
    }
  }
  ...
  @Override public void readFully(OkioBuffer sink, long byteCount) throws IOException {
    try {
      require(byteCount);
    } catch (EOFException e) {
      // The underlying source is exhausted. Copy the bytes we got before rethrowing.
  ...
  @Override public long readAll(OkioSink sink) throws IOException {
    ...
    while (source.read(buffer, OkioSegment.SIZE) != -1) {
  ...
  @Override public void skip(long byteCount) throws IOException {
    if (closed) throw new IllegalStateException("closed");
    while (byteCount > 0) {
      if (buffer.size == 0 && source.read(buffer, OkioSegment.SIZE) == -1) {
        throw new EOFException();
      }
  ...
  @Override public long indexOf(byte b, long fromIndex, long toIndex) throws IOException {
    ...
    while (fromIndex < toIndex) {
      ...
      if (lastBufferSize >= toIndex || source.read(buffer, OkioSegment.SIZE) == -1) return -1L;
  ...
  @Override public long indexOf(OkioByteString bytes, long fromIndex) throws IOException {
    ...
    while (true) {
      ...
      long lastBufferSize = buffer.size;
      if (source.read(buffer, OkioSegment.SIZE) == -1) return -1L;
  ...
  @Override public long indexOfElement(OkioByteString targetBytes) throws IOException {
    return indexOfElement(targetBytes, 0);
  }

  @Override public long indexOfElement(OkioByteString targetBytes, long fromIndex) throws IOException {
    if (closed) throw new IllegalStateException("closed");
  ...
  @Override public boolean rangeEquals(long offset, OkioByteString bytes) throws IOException {
    return rangeEquals(offset, bytes, 0, bytes.size());
  }

  @Override
  public boolean rangeEquals(long offset, OkioByteString bytes, int bytesOffset, int byteCount)
      throws IOException {
    if (closed) throw new IllegalStateException("closed");
  ...
  @Override public InputStream inputStream() {
    return new InputStream() {
      @Override public int read() throws IOException {
        ...
        if (buffer.size == 0) {
          long count = source.read(buffer, OkioSegment.SIZE);
          if (count == -1) return -1;
        }
      ...
      @Override public void close() throws IOException {
        OkioRealBufferedSource.this.close();
      }

      @Override public String toString() {
        return OkioRealBufferedSource.this + ".inputStream()";
      }
    };
  }
