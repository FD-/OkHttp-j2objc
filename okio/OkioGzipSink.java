 ...
 */
public final class OkioGzipSink implements OkioSink {
   ...
   */
  private final OkioDeflaterSink deflaterSink;
  ...
  public OkioGzipSink(OkioSink sink) {
    if (sink == null) throw new IllegalArgumentException("sink == null");
    this.deflater = new Deflater(DEFAULT_COMPRESSION, true /* No wrap */);
    this.sink = Okio.buffer(sink);
    this.deflaterSink = new OkioDeflaterSink(this.sink, deflater);
  ...
  private void writeHeader() {
    ...
    OkioBuffer buffer = this.sink.buffer();
  ...
  private void updateCrc(OkioBuffer buffer, long byteCount) {
    for (OkioSegment head = buffer.head; byteCount > 0; head = head.next) {
